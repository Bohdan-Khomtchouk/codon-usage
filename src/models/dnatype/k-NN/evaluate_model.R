...
load(file = "models/dnatype/k-NN/knn_model.RData") #yours will be different
load(file = "processed_data/1e3_5kingdoms/test_1e3_3dnatype.RData") #yours will be different

model <- knn.model
myInput_test_labels <- myInput_test$DNAtype #Kingdom labels only
...
calculate.AUC <- function(myInput_test){
  ...
  colnames(test.labels) <- c("0_true", "1_true", "2_true", "0_pred_knn", "1_pred_knn", "2_pred_knn")
