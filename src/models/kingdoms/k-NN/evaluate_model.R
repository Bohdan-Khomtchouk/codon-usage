## ----------------------------------------------------------------------------------- ##
## Program to evaluate k-nearest neighbors models using test data
## ----------------------------------------------------------------------------------- ##
...
load(file = "models/kingdoms/k-NN/knn_model.RData") #yours will be different
...
calculate.AUC <- function(myInput_test){
  ...
  colnames(test.labels) <- c("archaea_true", "bacteria_true", "eukaryote_true", "virus_true", "bacteriophage_true", "archaea_pred_knn", "bacteria_pred_knn", "eukaryote_pred_knn", "virus_pred_knn", "bacteriophage_pred_knn")
  
  model.roc <- multi_roc(test.labels)
  return(round(model.roc$AUC$knn$micro, 4))
}
