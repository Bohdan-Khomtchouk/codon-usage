## ----------------------------------------------------------------------------------- ##
## Program to evaluate random forest models using test data
## ----------------------------------------------------------------------------------- ##
...
myInput_test_preds <- predict(rf.model, newdata = myInput_test[])
...
calculate.AUC <- function(myInput_test){
  ...
  model.preds <- as.data.frame(predict(rf.model, newdata = myInput_test[, -1], type = "prob"))
  
  test.labels <- cbind(test.labels, model.preds)
  colnames(test.labels) <- c("archaea_true", "bacteria_true", "eukaryote_true", "virus_true", "bacteriophage_true", "archaea_pred_rf", "bacteria_pred_rf", "eukaryote_pred_rf", "virus_pred_rf", "bacteriophage_pred_rf")
  
  model.roc <- multi_roc(test.labels)
  return(round(model.roc$AUC$rf$micro, 4))
}
