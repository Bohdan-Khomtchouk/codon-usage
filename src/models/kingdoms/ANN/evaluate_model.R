## ----------------------------------------------------------------------------------- ##
## Program to evaluate neural net models using test data
## ----------------------------------------------------------------------------------- ##
...
load(file = "models/kingdoms/ANN/nnet_model.RData") #yours will be different
...
calculate.AUC <- function(myInput_test){
  ...
  colnames(test.labels) <- c("archaea_true", "bacteria_true", "eukaryote_true", "virus_true", "bacteriophage_true", "archaea_pred_nnet", "bacteria_pred_nnet", "eukaryote_pred_nnet", "virus_pred_nnet", "bacteriophage_pred_nnet")
  
  model.roc <- multi_roc(test.labels)
  return(round(model.roc$AUC$nnet$micro, 4))
}
